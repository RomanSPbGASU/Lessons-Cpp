...
ellipse::ellipse() : secondradius(786 * 0.2)
{
	circle();
}
void ellipse::set_ellipse(COORD circlecenter, int rad, int secrad, COLORREF circlecolor)
{
	set_circle(circlecenter, rad, circlecolor);
	secondradius = secrad;
}
	...
	for (int i = 0; i < radius; i++)
	{
		for (int j = 0; j > -secondradius; j--)
		{
			if (  sqrt(pow(F1.X - i, 2) + pow(F1.Y - j, 2))
				...
				<= 2 * primeradius )
			{
				SetPixel(hDeviceContext, center.X + i, center.Y + j, color);
				SetPixel(hDeviceContext, center.X - i, center.Y + j, color);
				SetPixel(hDeviceContext, center.X + i, center.Y - j, color);
				SetPixel(hDeviceContext, center.X - i, center.Y - j, color);
			}
